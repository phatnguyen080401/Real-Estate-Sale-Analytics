...
logger = Logger('Batch-Total-Customer-By-Property-Type')

class BatchTotalCustomerByPropertyType:
  def __init__(self):
  ...
  def save_to_snowflake(self, batch_df):
    try:
      ...
      property_type_df = batch_df.select(col("property_type"), col("serial_number"))

      fill_null_df = property_type_df.na.fill("Unknown")

      total_customer_by_property_type_df = fill_null_df \
      ...
      total_customer_by_property_type_df = total_customer_by_property_type_df \
      ...
      logger.info(f"Save to table sale_batch.total_customer_by_property_type: {df_rows} rows")
...
if __name__ == '__main__':
  BatchTotalCustomerByPropertyType().run()